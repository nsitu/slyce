...
export class KTX2Assembler {
     ...
     * @param {Function} [onProgress] - Progress callback function(layerIndex, totalLayers)
     * @returns {Promise<ArrayBuffer>} - KTX2 file buffer with layered texture
     */
    static async encode(layerSource, expectedCount = null, onProgress = null) {
        ...
        for await (const layer of layerSource) {
            ...
            if (onProgress && expectedCount) {
                onProgress(layerIndex, expectedCount);
            }
        }
    ...
    /**
     * Encode multiple RGBA frames into a KTX2 2D array texture using parallel workers
     * This method collects all frames first, then encodes them in parallel
     ...
     * @returns {Promise<ArrayBuffer>} - KTX2 file buffer with layered texture
     */
    static async encodeParallel(frames, onProgress = null) {
        console.log(`[Parallel Array Encoder] Starting parallel encoding with worker pool...`);
        const startTime = performance.now();
        ...
        // Create worker pool
        const workerPool = new KTX2WorkerPool();
